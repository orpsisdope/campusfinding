...
exports.getItems = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, user_id, title, type, category, location, item_date,
...
exports.createItem = async (req, res) => {
  const values = {
    ...
    type: typeof req.body.type === "string" ? req.body.type.trim().toLowerCase() : "",
    ...
    item_date: typeof req.body.item_date === "string" ? req.body.item_date.trim() : "",
  ...
  try {
    const result = await pool.query(
      `INSERT INTO items
       ...
       RETURNING id, user_id, title, type, category, location, item_date,
                 description, contact, resolved, created_at`,
      [
        req.user.id,
        values.title,
  ...
  try {
    ...
    if (Number(item.user_id) !== Number(req.user.id)) {
      return res.status(403).json({
        message: "Only the person who created this report can mark it as resolved."
      });
    }
    ...
    if (result.rowCount === 0) {
      return res.status(403).json({
        message: "You are not allowed to resolve this report."
      });
    }
